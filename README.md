...
In it's current state it should be used for experimenting only. You should also only use it with the current version of [sfall](https://github.com/sfall-team/sfall "A set of engine modifications for the classic game Fallout 2 in the form of a DLL, which modifies executable in memory without changing anything in EXE file itself."), as thats what I'm testing it against and I noticed it caused a crash on start when using an older version.
